...
(5, 'Store no5', 'Tzanakaki', 25, 'Chania');
...
(1, 'Konstantinos', 'Vamvakas', '6974589745', 'Tsouderon 24', 'Athens', '605.34', '2018-12-13', 3),
(2, 'Mixail', 'Stauroulidakis', '6982124569', 'Samarias 12', 'Heraklion', '789.45', '2012-05-11', 2),
(3, 'Georgios', 'Papadopoulos', '6973254123', 'Minoos 127', 'Thessaloniki', '920.12', '2007-02-04', 4),
(4, 'Ioannis', 'Ziogas', '6942566878', 'Ag. Gianni 67', 'Chania', '1024.30', '2000-05-19', 1),
(5, 'Spiridon', 'Iliopoulos', '6942123124', 'Polutexniou 129', 'Patra','1101.74', '1998-07-21', 5),
(6, 'Konstantinos', 'Papadoulakis', '6985401112', 'Papanastasiou 128', 'Athens','980.45', '1999-08-21', 4),
(7, 'Nikolaos', 'Pavlou', '6932147898', 'EOK 24', 'Heraklion', '845.23', '2001-07-30', 3),
(8, 'Katerina', 'Lada', '6932556554', 'Skra 9', 'Thessaloniki', '974.55', '2007-04-05', 2),
(9, 'Maria', 'Georgiou', '6987102030', 'Dagli 34', 'Chania', '880.40', '2011-11-25', 5),
...
(1788, '2019-01-01', '2020-01-01', 1000, 178.68),
(1812, '2019-11-11', '2020-11-11', 1000, 179.96);
...
select distinct atix.id  , y.auid
from PelatesOdhgoi po , Ypallhloi up , ypeuthunos y , atuxhmata atix
where (po.fname = 'Georgios'  or up.fname = "Georgios" )
and y.yid = up.id and y.pid = po.id and atix.id = y.aid;


-- -- 3ο

select * from PelatesOdhgoi po , sundeetai s , Atuxhmata a
where s.pid = po.id and s.aid = a.id and a.location = 'heraklion' 
and a.cdate>'2009-01-01' and a.cdate<'2009-12-12';

select distinct fname po, lname po
from PelatesOdhgoi po,  Atuxhmata atux, Sundeetai sun
where(cdate>='2009-01-01'and cdate<='2009-12-31') and atux.location='Heraklion' and atux.id=aid and po.id=pid;
...
from Atuxhmata
where cdate>='2009-01-01' and cdate<='2009-12-31'
GROUP BY location
...
select yp.id, yp.sname, price
from asfalisthrio asf, anhkei an , ypokatasthmata yp 
where asf.id = an.aid and yp.id = an.yid and price = (select max(price)
														from asfalisthrio);

-- -- 6ο 

select fname, lname, city, hdate
from Ypallhloi
where city='Heraklion';